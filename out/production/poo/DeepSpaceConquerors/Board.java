...
public class Board {
	...
	// Variables
	ShipsCards[] shipsCards = new ShipsCards[NUMBER_OF_CARDS_IN_BOARD];
	ConstructionCards[] constructionCards = new ConstructionCards[NUMBER_OF_CARDS_IN_BOARD];
	int numberOfPlayers;
	int diceAFaces = 4;
	int diceAMinValue = 2;
	int diceAMaxValue = 5;
	...
	Planets[] planets;
	Players[] players;
	ShipsCards purchasedShipCard;
	ConstructionCards purchasedConstructionCard;
	Dice diceA;
	Dice diceB;
	Dice diceC;
	...
	public Board(int numberOfPlayers) {
		...
		Dice diceA = new Dice(diceAFaces, diceAMinValue, diceAMaxValue);
		Dice diceB = new Dice(diceBFaces, diceBMinValue, diceBMaxValue);
		Dice diceC = new Dice(diceCFaces, diceCMinValue, diceCMaxValue);
		this.diceA = diceA;
		this.diceB = diceB;
		this.diceC = diceC;
		Planets[] planets = new Planets[this.numberOfPlayers * PLANETS_BY_PLAYERS];
		Players[] players = new Players[this.numberOfPlayers];
	...
	public void addPlanets() {
		for (int i = 0; i < planets.length; i++) {
			Planets planet = new Planets(planetName[i]);
			planets[i] = planet;
		}
	}
	/**
	...
	public void attackTarget(int player) {
		...
		int otherOption = 0;
		String target;
		AttackShipCard attackShip = null;
		...
		for (int i = 0; i < planets.length; i++) {
			for (int x = 0; x < planets[0].getShipsCardsArray().length; x++) {
				...
				if (planets[i].getShipsCardsArray()[x] instanceof AttackShipCard) {
		...
		if (option == 1) {
			...
			for(int i = 0; i < planets[option].getShipsCardsArray().length; i++) {
				 ...
				 */
				if (planets[option].getShipCard(i).getOwner() == players[player].getPlayerName() && planets[option].getShipCard(option) instanceof AttackShipCard) {
					System.out.println("Tienes naves de ataque en ese planeta. Puedes atacar.");
					// Le preguntamos que nave quieres atacar.
					System.out.println("�Qu� nave quieres atacar?");
			...
			for(int i = 0; i < planets[option].getShipsCardsArray().length; i++) {
				if (planets[option].getShipCard(i).getOwner() == players[player].getPlayerName()) {
			...
			if(planets[otherOption].getShipCard(option) instanceof AttackShipCard) {
				// Una vez elegida a una nave de ataque la asignamos a una variable.
				attackShip = (AttackShipCard)planets[otherOption].getShipCard(option);
			}
			...
			for(int i = 0; i < planets.length; i++) {
				...
				if (planets[i].getName() == target) {
					for (x = 0; x < planets[i].getShipsCardsArray().length && !attacked; x ++) {
						...
						if (planets[i].getShipsCardsArray()[x] instanceof AttackShipCard && planets[i].getShipsCardsArray()[x].getOwner() == players[player].getPlayerName()) {
							// La asignamos a una variable.
							attackShip = (AttackShipCard)planets[i].getShipsCardsArray()[x];
							// Y atacamos.
	...
	public void buyShipCard(int player) {
		...
		purchasedShipCard = shipsCards[option - 1];
		selectPlanetToAddShip(purchasedShipCard, player);
		shipsCards[option - 1] = null;
	}
	...
	public void checkGold(int option, int player) {
		...
		if (players[player].getGold() >= shipsCards[option - 1].price) {
			for (int i = 0; i < planets.length; i++) {
				if (players[player].getPlayerName().equals(planets[i].getOwner())) {
					System.out.printf("Este planeta es tuyo: %s \n", planets[i].getName());
					players[player].setGold(players[player].getGold() - shipsCards[option - 1].price);
				}
			}
		}
	}
	 ...
	 * @param player - Posici�n del jugador en el array de jugadores
	 */
	public void selectPlanetToAddShip(ShipsCards purchasedShipCard, int player) {
			...
			if (planets[i].getName() == selectedPlanet && planets[i].getOwner() == players[player].getPlayerName()) {
				for (int x = 0; x < planets[i].getShipsCardsArray().length && !added; x++) {
					if (planets[i].getShipCard(x) == null) {
						...
						planets[i].getShipsCardsArray()[x].setOwner(players[player].getPlayerName());
					}
				}
			}
			else {
	...
	public void buyConstructionCard(int player) {
		...
		if (option == 1) {
			...
			case 1:
				...
				minedPerShift = diceA.throwDace();
				IronMineCards ironMine = new IronMineCards(MINE_PRICE, minedPerShift);
				selectPlanetToAddMine(ironMine, player);
				break;
			case 2:
				// Creamos una mina de piedra.
				minedPerShift = diceA.throwDace();
				StoneMineCards stoneMine = new StoneMineCards(MINE_PRICE, minedPerShift);
				selectPlanetToAddMine(stoneMine, player);
				break;
			case 3:
				// Creamos una mina de combustible.
				minedPerShift = diceA.throwDace();
				FuelMineCards fuelMine = new FuelMineCards(MINE_PRICE, minedPerShift);
	...
	public void generateMaterials() {
		...
		for (int i = 0; i < planets.length; i++) {
			...
			for (int x = 0; x < planets[i].getMineCardsArray().length; x++) {
				...
				if(planets[i].mineCardsArray[x] instanceof StoneMineCards) {
					planets[i].setStone(planets[i].mineCardsArray[x].getMinedPerShift());
				}
				else if(planets[i].mineCardsArray[x] instanceof FuelMineCards) {
					planets[i].setFuel(planets[i].mineCardsArray[x].getMinedPerShift());
				}
				else if(planets[i].mineCardsArray[x] instanceof StoneMineCards) {
		...
		for (int i = 0; i < planets.length; i++) {
			if (planets[i].getName() == selectedPlanet && planets[i].getOwner() == players[player].getPlayerName()) {
				if(planets[i].getShield() == null) {
					int variableHealth = diceA.throwDace();
					ProtectiveShieldCards protectiveShield = new ProtectiveShieldCards(SHIELD_PRICE, variableHealth);
	 ...
	 * @param player - Posici�n del jugador en el array de jugadores.
	 */
	public void selectPlanetToAddMine(MineConstructionCards constructionCard, int player) {
	 ...
	 * @param player - Posici�n del jugador en el array de jugadores.
	 */
	public void selectPlanetToAddShield(ProtectiveShieldCards protectiveShield, int player) {
	...
	public void generateDeckOfShips() {
		for (int i = 0; i < NUMBER_OF_CARDS_IN_BOARD; i++) {
			if (shipsCards[i] == null) {
				int randomNum = (int) (Math.random() * (1 - 4) + 4);
				switch (randomNum) {
				case 1:
					AttackShipCard attackShip = new AttackShipCard(attackShipCardPrice);
					shipsCards[i] = attackShip;
					break;
				case 2:
					CargoShipCard cargoShip = new CargoShipCard(cargoShipCardPrice);
					shipsCards[i] = cargoShip;
					break;
				case 3:
					TranportShipCards transportShip = new TranportShipCards(transportShipCardPrice);
					shipsCards[i] = transportShip;
					break;
				}
			}
		}
	...
	public void printDeckOfShips() {
		for (int i = 0; i < NUMBER_OF_CARDS_IN_BOARD; i++) {
			System.out.println(shipsCards[i].getClass());
		}
	}
	...
	public void printDeckOfConstructions() {
		for (int i = 0; i < NUMBER_OF_CARDS_IN_BOARD; i++) {
			System.out.println(constructionCards[i].getClass());
		}
	}
